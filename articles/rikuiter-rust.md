...
find は条件に合う要素を見つけた時点で止まるため、見つかった後もイテレータは途中から再開できます。
...
新しい要素を入れてしまいました。`mem::take`は、変数の中身を取り出して、その変数を型のデフォルト値で置き換える関数です。ここでは`remaining`を取り出して`0`にリセットし、取り出した値を`n`に代入しています。
...
でも、構造体に制約を追加したくありませんねー。`Debug`があってもなくても動作には影響しないためです。
