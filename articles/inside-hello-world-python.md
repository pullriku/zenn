...
1. コンパイル
   Pythonソースコードをバイトコードに変換します。[^compile]バイトコードはPython仮想マシンが理解できる中間表現です。
1. 実行
...
`CALL`が実行された瞬間、制御はPythonのバイトコードインタプリタから離れ、C言語で実装された`print`関数の実体へと移ります。[^bound]ここが、PythonとC言語の世界の境界線です。
...
PythonもただのCプログラムであることを強調するために、`main`関数から追っていきます。
...
また、各コードブロックの上部にファイルパスと、その関数の呼び出し階層を示しておきます。実装を辿る際の参考にしてください。

### 組み込みモジュールの登録

まず、組み込みモジュール（`print`関数を含む`builtins`モジュール）が初期化され、インタプリタに登録されます。

```c:Python/pylifecycle.c | main > Py_BytesMain > pymain_main > pymain_init > Py_InitializeFromConfig > pyinit_core > pyinit_config > pycore_interp_init > pycore_init_builtins
...
```c:Python/bltinmodule.c | pycore_init_builtins > _PyBuiltin_Init
PyObject *
